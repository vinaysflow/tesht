...
function FactorRow({ label, val, max = 25, note }) {
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      <span className="w-36 text-tesht-muted shrink-0">{label}</span>
      <div className="flex-1 bg-tesht-dark rounded-full h-1.5 max-w-[120px]">
        <div className="bg-tesht-teal h-1.5 rounded-full" style={{ width: `${pct}%` }} />
      </div>
      <span className="w-10 text-right font-mono text-slate-300">{val}/{max}</span>
      {note && <span className="text-tesht-muted">{note}</span>}
    </div>
  )
}
...
export function TrustTimeline({ trustHistory }) {
  if (!trustHistory?.length) return <div className="text-tesht-muted text-sm">No trust history yet.</div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-tesht-card border border-tesht-border rounded-xl p-4">
        <h3 className="text-sm font-bold text-tesht-teal mb-4">Trust Score Over Session</h3>
      ...
      <div className="bg-tesht-card border border-tesht-border rounded-xl p-4 overflow-x-auto">
        <h3 className="text-sm font-bold text-slate-400 mb-3">Per-Request Breakdown</h3>
        <table className="w-full text-xs font-mono">
          <thead>
            <tr className="text-tesht-muted border-b border-tesht-border">
          ...
          <tbody>
            {trustHistory.map((e, i) => (
              <tr key={i} className="border-b border-tesht-border/50">
                <td className="py-1 text-tesht-muted">{i + 1}</td>
                <td className="py-1 text-slate-300">{e.tool || '—'}</td>
                <td className="py-1 text-tesht-teal font-bold">{e.trustScore}</td>
        ...
        return (
          <div className="bg-tesht-card border border-tesht-border rounded-xl p-4">
            ...
            <div className="space-y-2">
              ...
              <div className="border-t border-tesht-border my-2" />
