...
function CredCard({ cred, idx }) {
  ...
  return (
    <div className={`border rounded-lg p-3 mb-2 ${colorClass.split(' ')[0]} bg-tesht-dark`}>
      <div className="flex items-center justify-between cursor-pointer" onClick={() => setExpanded(!expanded)}>
        <div>
          <span className={`text-xs font-bold px-2 py-0.5 rounded border mr-2 ${colorClass}`}>
            {cred.type}
          </span>
          <span className="text-xs text-tesht-muted">Credential {idx}</span>
        </div>
        <span className="text-tesht-muted text-xs">{expanded ? '▲ collapse' : '▼ expand'}</span>
      </div>
      ...
      {expanded && (
        <div className="mt-3 space-y-1 text-xs font-mono">
          ...
          <details className="mt-2">
            <summary className="text-tesht-muted cursor-pointer">Raw JWT</summary>
            <pre className="mt-1 text-xs text-tesht-muted break-all whitespace-pre-wrap">{cred.raw}</pre>
          </details>
        </div>
      )}
    </div>
...
function Row({ label, val }) {
  return (
    <div className="flex gap-2">
      <span className="text-tesht-muted w-24 shrink-0">{label}</span>
...
export function VPViewer({ vpToken }) {
  if (!vpToken) return <div className="text-tesht-muted text-sm">No VP data yet.</div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-tesht-card border border-tesht-border rounded-xl p-4">
        <h3 className="text-sm font-bold text-tesht-teal mb-3">Verifiable Presentation</h3>
